...
const finiteSegment = function (ax, point, upperBound, lowerBound) {
  ...
  let a = axis(point, ax),
    ...
    arr = [];
  s.forEach((num, i) => {if (a.indexOf(num) !== -1) arr.push(num)});
  return arr;
};

// slices all common elements between segmentIterator and axis
// to return a segment of points that definitely exist
// within {bound} of {point} along {ax} axis.  If the point doesn't
// exist (is out of bounds), we modulo to the opposite end of the
// segment (visualize the tunnels in Ms. Pac-Man)
const infiniteSegment = function (ax, point, upperBound, lowerBound) {
  lowerBound = lowerBound || -upperBound;
  let a = axis(point, ax),
    s = possibleSegment(ax, point, upperBound, lowerBound),
    arr = [],
  ...
  s.forEach((num, i) => {
    if (a.indexOf(num) !== -1) {
      ...
      greaterThanMax.push(num);
    } else if (num <= min) {
      lessThanMin.push(num);
    }
  });
  return [
    ...lessThanMin
      .sort((x, y) => Math.abs(x) > Math.abs(y))
      .map((_, i) => a.slice(-i - 1, i ? -i : undefined)[0]),
    ...arr,
    ...greaterThanMax
      .sort((x, y) => Math.abs(x) > Math.abs(y))
      .map((_, i) => a.slice(i, i + 1)[0]),
  ].reduce((output, num) => {
    return output.indexOf(num) === -1 ? [...output, num] : output;
  }, []);
};
...
const region = function * (coord, dimension) {
  ...
  for (let n of infiniteSegment(dimension, coord, 1, -1)) {
    yield * region(n, dimension + 1);
  }
};
